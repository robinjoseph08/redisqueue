...
func TestNewConsumer(t *testing.T) {
	...
	t.Run("creates a new consumer", func(tt *testing.T) {
		c, err := NewConsumerWithContext(ctx)
...
func TestNewConsumerWithOptions(t *testing.T) {
	ctx := context.Background()
	t.Run("creates a new consumer", func(tt *testing.T) {
		c, err := NewConsumerWithContextOptions(ctx, &ConsumerOptions{})
		require.NoError(tt, err)
	...
	t.Run("bubbles up errors", func(tt *testing.T) {
		_, err := NewConsumerWithContextOptions(ctx, &ConsumerOptions{
...
func TestRun(t *testing.T) {
	...
	t.Run("calls the ConsumerFunc on for a message", func(tt *testing.T) {
		...
		// enqueue a message
		err = p.EnqueueWithContext(ctx, &Message{
			Stream: tt.Name(),
	...
	t.Run("doesn't reclaim if there is no VisibilityTimeout set", func(tt *testing.T) {
		...
		err = p.EnqueueWithContext(ctx, msg1)
		require.NoError(tt, err)
		...
		err = p.EnqueueWithContext(ctx, msg2)
		require.NoError(tt, err)
